...
function detectKycMismatch({ accounts, transactions, pattern }) {
  ...
  for (const account of accounts) {
    ...
    // Provide a baseline income of 5L if none is declared to avoid zero-division issues
    const effectiveAnnualIncome = annualIncome > 0 ? annualIncome : 500000;
...
export function detectFraudAlerts({ accounts, transactions, patterns, targetAccountIds }) {
  ...
  if (targetAccountIds && targetAccountIds.length > 0) {
    ...
    const filteredAlerts = uniqueAlerts.filter(alert => 
      alert.involved_accounts.some(acc => targetSet.has(acc))
    );
    return filteredAlerts.sort((left, right) => right.confidence_score - left.confidence_score);
  }
